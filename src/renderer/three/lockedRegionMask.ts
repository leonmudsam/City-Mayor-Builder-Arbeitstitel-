...
import { DataTexture, NearestFilter, RedFormat, UnsignedByteType, type Material } from 'three';
...
export function patchLockedRegionTint(
  material: Material,
  options: { taps?: number; cacheKey?: string } = {},
): void {
  ...
  // Vorhandene Hooks werden GEKETTET, nicht ersetzt: Der Wind-Shader der
  // Vegetation und der Splat-Shader des Bodens hängen ebenfalls hier.
  ...
  material.customProgramCacheKey = () => `${previousKey?.() ?? ''}|cmb-lock-1-${cacheKey}-${taps}`;
