...
function tileTarget(tx: number, ty: number): number {
  ...
  let h = base + hills * amp;
  if (type === 'mountain') {
    // Extra rugged detail so peaks vary instead of forming a flat plateau.
    h += fbm(tx * 0.17 + 21, ty * 0.17 + 13) * amp * 0.6;
  }
  return h;
}
