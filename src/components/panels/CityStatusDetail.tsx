...
// The full city control room, opened from the persistent status widget's
// "Details ansehen" (§3): actionable alerts first (each clicks through to the
// fix), then the complete needs breakdown.
export function CityStatusDetail() {
  const game = useGame();
  ...
  if (state.quests.active.some((q) => q.claimable)) {
    alerts.push({
      ...
      actionLabel: t('ui.claim'),
      // The citizen-requests panel is always visible on the right; just close
      // the detail overlay so the claimable card is in reach.
      onAction: () => setPanel(undefined),
    });
  }
