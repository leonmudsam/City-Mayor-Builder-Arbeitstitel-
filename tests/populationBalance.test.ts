...
describe('Aktives Spiel: keine Offline-Produktion (§ C7/§26)', () => {
  it('produziert nur im Live-Tick, nicht im Offline-Catch-up', () => {
    ...
    expect(controller.placeBuilding('sawmill', at(1, 6).x, at(1, 6).y).ok).toBe(true);
    // Bauzeit läuft auch offline ab (non-live) → Sägewerk wird aktiv.
    ...
    // Offline-Tick: KEINE Holzproduktion.
    const woodBefore = controller.state.resources.wood;
    controller.update(T0 + HOUR + 60_000, false);
    expect(controller.state.resources.wood).toBe(woodBefore);

    // Live-Tick: Holz steigt.
