...
describe('income breakdown (§5)', () => {
  ...
  it('aggregates commercial and industrial revenue bases from buildings', () => {
    ...
    controller.state.resources = { money: 200_000, wood: 500, stone: 500, food: 100, freshwater: 0, planks: 400, cut_stone: 400 };
...
describe('logistics & workplaces', () => {
  it('a depot boosts the output of production buildings in range (§1 supply chain)', () => {
    ...
    controller.state.resources = { money: 500_000, wood: 1_000, stone: 1_000, food: 1_000, freshwater: 0, planks: 400, cut_stone: 400 };
  ...
  it('a depot also speeds up an active operation (§A6/A7)', () => {
    const build = (withDepot: boolean) => {
      ...
      setLevel(controller, 7);
      flattenTerrain(controller);
      controller.state.resources = { money: 900_000, wood: 1_000, stone: 1_000, food: 1_000, freshwater: 0, planks: 400, cut_stone: 400 };
      for (let dx = 5; dx <= 13; dx++) controller.placeBuilding('road', at(dx, 5).x, at(dx, 5).y);
  ...
  it('an office supplies a large block of jobs (§ Arbeitsversorgung)', () => {
    ...
    flattenTerrain(controller);
    controller.state.resources = { money: 1_200_000, wood: 1_000, stone: 1_000, food: 1_000, freshwater: 0, planks: 400, cut_stone: 400 };
...
describe('energy grid (MVP 2)', () => {
  it('a power plant feeds the grid while buildings draw power', () => {
    ...
    flattenTerrain(controller);
    controller.state.resources = { money: 1_500_000, wood: 2_000, stone: 2_000, food: 1_000, freshwater: 0, planks: 400, cut_stone: 400 };
...
describe('emergency services (MVP 2)', () => {
  it('a police station covers the homes within its safety radius', () => {
    ...
    flattenTerrain(controller);
    controller.state.resources = { money: 1_000_000, wood: 2_000, stone: 2_000, food: 1_000, freshwater: 0, planks: 400, cut_stone: 400 };
...
describe('housing model (§6)', () => {
  it('derives resident capacity from units × max residents per unit', () => {
    ...
    flattenTerrain(controller);
    controller.state.resources = { money: 5_000_000, wood: 3_000, stone: 3_000, food: 1_000, freshwater: 0, planks: 400, cut_stone: 400 };
...
describe('building upgrades (v0.17)', () => {
  it('gates densification upgrades behind city level and grows capacity on the same tile', () => {
    const { controller } = newController();
    flattenTerrain(controller);
    controller.state.resources = { money: 3_000_000, wood: 3_000, stone: 3_000, food: 1_000, freshwater: 0, planks: 400, cut_stone: 400 };
    controller.placeBuilding('house_small', at(3, 6).x, at(3, 6).y);
